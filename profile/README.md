...
|---|---|---|
| [NTI-client](https://github.com/FrefLabs/NTI-client) | Java 8 · Swing | Interfaz de escritorio para el usuario |
| [NTI-server](https://github.com/FrefLabs/NTI-server) | Java 8 · HTTP embebido | Servidor REST central, gestión de la cola de entrenamientos |
| [NTI-train](https://github.com/FrefLabs/NTI-train) | Java 8 · Neuroph | Cliente de entrenamiento distribuido |
...
El sistema está compuesto por cuatro módulos independientes organizados en una arquitectura distribuida por capas, comunicados exclusivamente a través de interfaces de red:

```mermaid
flowchart TD
  %% Definición de Nodos principales
    User([Usuario])
    Client[NTI-client<br/>Interfaz Swing / Escritorio<br/>]
    Server[NTI-server<br/>Servidor Central / Java<br/>]
    Gateway[NTI-gateway<br/>API Python / FastAPI <br/>]

    %% Subgrafica para denotar el paralelismo de los trabajadores
    subgraph Cluster_Train [Nodos de Cómputo Distribuido]
    ...
    class TrainA,TrainB worker;

```

El cliente solo habla con el servidor. El servidor coordina el gateway (para datos financieros) y los dispositivos de entrenamiento (NTI-train), los cuales no se conocen entre sí.
...
### 2. Obtener datos (NTI-server → NTI-gateway)
...
Si Yahoo Finance devuelve un error 429 (rate limit), el gateway redistribuye la tarea a otro Nodo disponible vía WebSocket en lugar de fallar.
...
Una vez finalizado el entrenamiento, el modelo aparece en la interfaz con su precisión, métricas detalladas, historial de predicciones y los parámetros usados.
...
**Entradas:** 19 features técnicos por fila del dataset (configurables):
...
**Métricas calculadas sobre el test set (con valores desnormalizados):**
...
Dashboard principal. Muestra el gráfico de velas del ticker activo, precio actual, recomendación diaria (COMPRAR / MANTENER / VENDER), precio de cierre predicho por el modelo seleccionado, métricas de mercado fundamentales, ranking de los modelos más precisos y noticias recientes de la empresa.

### Pantalla de Modelos
![Modelos](https://raw.githubusercontent.com/FrefLabs/NTI-client/refs/heads/master/screenshots/Pantalla%20de%20Modelos.png)

Lista de modelos disponibles con buscador en tiempo real. Muestra el modelo activo con su precisión, MAE y rango de entrenamiento. Desde acá se puede crear un nuevo modelo o ver el detalle de cualquiera existente.

### Detalle de modelo
![Detalle](https://raw.githubusercontent.com/FrefLabs/NTI-client/refs/heads/master/screenshots/Pantalla%20de%20Modelo%20Seleccionado%20(Modelos).png)

Vista completa de un modelo: features usadas, resultados sobre el período de entrenamiento (MSE, RMSE, MAE, R², etc.), resultados desde la fecha fin hasta hoy (para medir performance real post-entrenamiento), y los parámetros de configuración usados.

### Historial
![Historial](https://raw.githubusercontent.com/FrefLabs/NTI-client/refs/heads/master/screenshots/Pantalla%20de%20Historial.png)
...
**NTI-client / NTI-server / NTI-train:** Java 8 · NetBeans · Apache Ant · Neuroph 2.98 · JFreeChart 1.0.19 · OpenCSV 5.9 · MariaDB Java Client · JSON Simple

**NTI-gateway:** Python 3.9+ · FastAPI 0.104 · Uvicorn · WebSockets · yfinance · pandas · numpy · scikit-learn · deep-translator

**APIs externas:** Finnhub (noticias y cotizaciones) · Yahoo Finance (datos históricos, vía gateway) · ExchangeRate (conversión de divisas)
